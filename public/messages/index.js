...
const backendAPIs = 'http://localhost:4000/message';
...
async function dom() {
	...
	const response = await axios.get(
		`${backendAPIs}/getMessage/${groupId}?lastMessageId=${lastMessageId}`,
...
function showOtherMessgeOnScreen(obj) {
	...
	if (
		obj.message.indexOf('https://') == 0 ||
		obj.message.indexOf('http://') == 0
	) {
		chat.innerHTML += `
        <li class="you">
            <div class="entete">
...
async function sendMessage(event) {
	try {
		...
		const response = await axios.post(
			`${backendAPIs}/sendMessage/${groupId}`,
			{ message: message },
			{ headers: { Authorization: token } }
		);
		console.log(response.data);
...
function time(string) {
	...
	return time_object.toLocaleTimeString('en-US', {
		hour: 'numeric',
		minute: 'numeric',
		hour12: true,
	});
...
function date(string) {
	...
	const yesterday_date = `${today.getDate() - 1}-${
		today.getMonth() + 1
	}-${today.getFullYear()}`;
	const gettingDate = `${date_object.getDate()}-${
...
function displayNameForAdmin(user) {
	...
	if (user.email == userEmail) {
		document.getElementById(`name${userEmail}`).style.color =
			'rgb(186, 244, 93)';
	}
}
...
async function makeAdmin(email) {
	...
	try {
		...
		document
			...
			.setAttribute('onClick', `removeAdmin('${email}')`);
...
async function deleteUser(email) {
	if (confirm('Are you sure')) {
		try {
			//console.log(email);
			const response = await axios.post(
				`${backendAPIs}/deleteUser/${groupId}`,
				{ email: email },
				{ headers: { Authorization: token } }
			);
			//console.log(response);
...
async function removeAdmin(email) {
	try {
		if (confirm(`Are you sure ?`)) {
			//console.log(email);
			const response = await axios.post(
				`${backendAPIs}/removeAdmin/${groupId}`,
				{ email: email },
				{ headers: { Authorization: token } }
			);
			//console.log(response);
			...
			document
				.getElementById(`admin${email}`)
				.setAttribute('onClick', `makeAdmin('${email}')`);
