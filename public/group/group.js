...
const backendAPIs = 'http://localhost:4000/group';

//getting all groups on screen
window.addEventListener('DOMContentLoaded', async () => {
	const response = await axios.get(`${backendAPIs}/getGroup`, {
	...
	const result = await axios.post(`${backendAPIs}/getAllGroups`, {
		groupsId: groupsId,
	});
...
otherGroups.addEventListener('click', async (e) => {
	if (e.target.classList.contains('join')) {
		...
		const id = e.target.parentNode.id;
		const result = await axios.get(`${backendAPIs}/join/${id}`, {
			headers: { Authorization: token },
		});
		// console.log(result);
