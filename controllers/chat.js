		...
		if (isStringValid(message)) {
			...
			if (!isUserInGroup) {
				return res.status(400).json({
					success: false,
					message: 'You are no longer in group now !',
				});
			}
...
const addUser = async (req, res, next) => {
	try {
		...
		if (isStringValid(email)) {
			...
			const checkUserIsAdmin = await UserGroup.findOne({
				where: { userId: req.user.id, groupId: groupId },
			});
			...
			if (req.user.email == email) {
				return res.status(500).json({
					success: false,
					message: `Admin is already in group !`,
				});
			}
	...
	try {
		...
		if (isStringValid(email)) {
			...
			if (!user) {
				return res.status(400).json({
					success: false,
					message: `This user doesn't exist in database !`,
				});
			}
			...
			res.status(200).json({
				success: true,
				message: `Now ${user.name} is Admin of this group !`,
			});
		}
	} catch (err) {
		console.log(err);
...
const deleteUser = async (req, res, next) => {
	try {
		...
		const checkUser = await UserGroup.findOne({
			where: { groupId: groupId, userId: req.user.id },
		});
		if (!checkUser) {
			return res.status(400).json({
				success: false,
				message: `You are no longer in group !`,
			});
		}
		...
		if (checkUser.isAdmin == false) {
			return res.status(400).json({
				success: false,
				message: `Only admin can delete members from groups !`,
			});
		}
		...
		if (req.user.email == email) {
			if (allAdmins.length > 1) {
				...
				return res.status(200).json({
					success: true,
					message: `User has been deleted from group !`,
				});
			} else {
				return res.status(400).json({
					success: false,
					message: `Make another user as an Admin !`,
				});
			}
		}
...
const removeAdmin = async (req, res, next) => {
	...
	try {
		...
		res.status(200).json({
			success: true,
			message: `User ${user.name} is no longer admin now !`,
		});
	} catch (err) {
		console.log(err);
		res.status(400).json({
